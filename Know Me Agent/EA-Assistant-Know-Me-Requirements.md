...
### Read Traveler Preferences Endpoint

```
GET /know-me/v1/users/{userId}?context=booking
```

**Response includes:**
- Personal information (name, email, contact details)
- Travel preferences (flight, hotel, ground transport)
- Loyalty programs (with status/tier)
- Behavioral patterns (with confidence scores)
- Profile completeness validation

### Validate Profile Completeness Endpoint

```
GET /know-me/v1/users/{userId}/validation?tripType=international
```

**Response includes:**
- Required fields status (present/missing/expired)
- Warning messages for expiring credentials
- Recommendations for optional fields
- Priority levels (critical/recommended/optional)

### Generate Work Items Endpoint

```
GET /know-me/v1/arrangers/{arrangerId}/work-items
```

**Response includes:**
- List of travelers with missing/expired profile data
- Field name, reason, priority
- Traveler context (upcoming trips requiring the field)
- Direct links to profile update UI

### Update Profile Fields Endpoint

```
PATCH /know-me/v1/users/{userId}/profile
```

**Request body:**
```json
{
  "updatedBy": "arranger-user-id",
  "fields": {
    "passport.number": "X12345678",
    "passport.expirationDate": "2028-06-15",
    "loyaltyPrograms.marriott": "MB123456789"
  }
}
```

**Response includes:**
- Success/failure for each field update
- Audit trail entry created
- Fields that require traveler self-service
